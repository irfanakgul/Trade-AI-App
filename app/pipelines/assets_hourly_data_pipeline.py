...
class OandaHourlyDataPipelineFlags:
    ...
    converted_table:str = 'converted_daily_dataset_assets'
...
async def run_oanda_hourly_data_pipeline(repo, flags: OandaHourlyDataPipelineFlags,exchange):
    print(
        "\n[INGESTION] ASSETS pipeline started... "
    ...
    if flags.ingest:
        ...
        print(f"[INGESTION] ASSETS symbol_count={len(symbols)}")
        ...
        print("❌ [INGESTION] ASSETS skipped")
    ...
    if flags.trim_history:
        print(F"[TRIM365] ASSETS TRIM365 started...\n")

        before = repo.count_rows(schema="bronze", table="synced_working_assets_hourly")
        print(f"[TRIM365] ASSETS rows before trim: {before}")

        deleted = repo.trim_history_by_peak_or_lookback_ts(
            schema="bronze",
            table="synced_working_assets_hourly",
            symbol_col="SYMBOL",
            ts_typed_col="TS",
            high_col="HIGH",
            lookback_days=365,
        ...
        print(
            f"[TRIM365] ASSETS trim365 completed. deleted_rows={deleted} "
            f"{datetime.now().strftime('%d-%m-%Y %H:%M')}"
        )

        after = repo.count_rows(schema="bronze", table="synced_working_assets_hourly")
        print(f"[TRIM365] ASSETS rows after trim: {after}")
    else:
        print("❌ [TRIM365] ASSETS trim skipped")

    # ----------------------------------------------------------
    # 4) BUILD FOCUS DATASET
    # ----------------------------------------------------------
    if flags.build_focus_dataset:
        print("[IND-FOCUS] ASSETS dataset build started...")

        stats = repo.build_frvp_focus_dataset(
            source_schema="bronze",
            source_table="synced_working_assets_hourly",
            target_schema="silver",
            target_table="indicators_assets_focus_dataset",
            ts_col="TS",
            high_col="HIGH",
            exchange=exchange,
            min_trading_days=20,
        ...
        repo.update_focus_symbol_scope(
            ...
            compare_table='indicators_assets_focus_dataset',
        ...
        print(
            f'[IND-FOCUS] ASSETS Focus dataset built. '
            f'symbols: {stats["before_symbols"]} -> {stats["after_symbols"]}, '
            f'rows: {stats["before_rows"]} -> {stats["after_rows"]} '
            f'{datetime.now().strftime("%d-%m-%Y %H:%M")}')
        
    else:
        print("❌ [IND-FOCUS] ASSETS dataset build skipped!")

    # ----------------------------------------------------------
    # 5) DQ
    # ----------------------------------------------------------
    if flags.run_dq:
        print("[DQ] ASSETS starting...")

        dq = DQV2Service(repo)

        dq_run_cfg = DQRunConfig(
            job_name="assets_hourly_data_pipeline",
            active_table="dq_check_overview_assets",
        ...
        tables = [
            DQTableConfig(
                exchange="OANDA",
                schema_name="raw",
                table_name="assets_hourly_archive",
                interval="hourly",
                ts_col="TS",
            ...
            DQTableConfig(
                exchange="OANDA",
                schema_name="bronze",
                table_name="synced_working_assets_hourly",
                interval="hourly",
                ts_col="TS",
                timestamp_col="TIMESTAMP",
                symbol_col="SYMBOL",
            ...
            DQTableConfig(
                exchange="OANDA",
                schema_name="silver",
                table_name="indicators_assets_focus_dataset",
                interval="hourly",
                ts_col="TS",
                timestamp_col="TIMESTAMP",
                symbol_col="SYMBOL",
    ...
    #--------------------------
    if flags.bar_status:
        print("[IND-BAR_STATUS] assets started for exchange={exchange}")
        ...
        print(f"❌ [IND-BAR_STATUS] assets skipped for exchange={exchange}")
    ...
    if flags.run_frvp:
        print(f"[IND-FRVP] assets started ({exchange})...")
        ...
        svc.run(
            ...
            frvp_source_table='indicators_assets_focus_dataset',
        ...
        print(f"❌ [IND-FRVP] assets skipped for exchange={exchange}")

    #---------------------------------
    # IND-3) run_convert_daily
    #---------------------------------
    if flags.run_convert_daily:
        print(f"[IND-CONVERT_DAILY] assets started ({exchange})...")
        ...
        stats = repo.build_converted_daily_for_indicators(
            ...
            target_table=flags.converted_table,#'converted_daily_dataset_assets',
        )
        print(
        ...
        print(f"❌ [IND-CONVERT] assets skipped for exchange={exchange}", flush=True)
    ...
    if flags.run_ema_ind:
        print(f"[IND-EMA] assets started ({exchange})...")
        ...
        print('❌ [IND-EMA] assets skipped!')

    #---------------------------------
    # IND-5) WVAP CALC
    #---------------------------------
    if flags.run_vwap_ind:
        print(f"[IND-VWAP] assets started ({exchange})...")
    ...
    if flags.run_rsi_ind:
        print(f"[IND-RSI] assets started ({exchange})...")
        ...
        print('❌ [IND-RSI] assets skipped!')

    #---------------------------------
    # IND-7) MFI CALC
    #---------------------------------
    if flags.run_mfi_ind:
        print(f"[IND-MFI] assets started ({exchange})...")
    ...
    if flags.run_combined_indicators:
        ...
        svc.run(
            exchange=exchange,
            target_schema='gold',
            target_table='assets_master_combined_indicators',
            log_schema='logs',
            log_table='log_assets_master_combined_indicators',
    ...
    if flags.run_master_score:
        print(f"[MASTER-SCORE] started: ASSETS...")
        ...
        svc.run(
            exchange=exchange,
            input_schema="gold",
            input_table=f"assets_master_combined_indicators",
            output_schema="gold",
            output_table=f"assets_evaluation_master_score",
            stop_loss_perc=3.0,
            entry_markup_perc=0.5,
            top_n=10,
            created_at=run_ts,
            send_telegram=True,
            telegram_title=f"CRYPTO TOP 10",
